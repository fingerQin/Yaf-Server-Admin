...
class Excel2007 extends BaseReader implements IReader {
        ...
        foreach ( $rels->Relationship as $rel ) {
            if ($rel["Type"] == "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument") {
                ...
                $relsWorkbook = simplexml_load_string(
                        // ~ http://schemas.openxmlformats.org/package/2006/relationships"
                        $this->securityScan($this->getFromZipArchive($zip, "$dir/_rels/" . basename($rel["Target"]) . ".rels")), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions());
    ...
    public function load($pFilename) {
        ...
        $wbRels = simplexml_load_string(
                // ~ http://schemas.openxmlformats.org/package/2006/relationships"
                $this->securityScan($this->getFromZipArchive($zip, "xl/_rels/workbook.xml.rels")), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions());
            ...
            switch ($rel["Type"]) {
                ...
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" :
                    ...
                    $xmlStrings = simplexml_load_string(
                            // ~ http://schemas.openxmlformats.org/spreadsheetml/2006/main"
                            $this->securityScan($this->getFromZipArchive($zip, "$dir/$xpath[Target]")), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions());
                    ...
                    if ($xmlWorkbook->sheets) {
                        foreach ( $xmlWorkbook->sheets->sheet as $eleSheet ) {
                            ...
                            $xmlSheet = simplexml_load_string(
                                    // ~ http://schemas.openxmlformats.org/spreadsheetml/2006/main"
                                    $this->securityScan($this->getFromZipArchive($zip, "$dir/$fileWorksheet")), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions());
                                    ...
                                    foreach ( $xmlSheet->autoFilter->filterColumn as $filterColumn ) {
                                        ...
                                        if ($filterColumn->filters) {
                                            ...
                                            foreach ( $filters->dateGroupItem as $dateGroupItem ) {
                                                $column->createRule()->setRule(
                                                        ...
                                                        ), (string) $dateGroupItem["dateTimeGrouping"])->setRuleType(\PHPExcel\Worksheet\AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DATEGROUP);
                                            }
                                        }
                                        ...
                                        if ($filterColumn->dynamicFilter) {
                                            ...
                                            foreach ( $filterColumn->dynamicFilter as $filterRule ) {
                                                $column->createRule()->setRule(
                                                        // Operator is undefined, but always treated as EQUAL
                                                        null, (string) $filterRule["val"], (string) $filterRule["type"])->setRuleType(\PHPExcel\Worksheet\AutoFilter\Column\Rule::AUTOFILTER_RULETYPE_DYNAMICFILTER);
                            ...
                            if (!$this->readDataOnly) {
                                ...
                                if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
                                    $relsWorksheet = simplexml_load_string(
                                            // ~ http://schemas.openxmlformats.org/package/2006/relationships"
                                            $this->securityScan($this->getFromZipArchive($zip, dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions());
                                    ...
                                    if ($zip->locateName(dirname("$dir/$fileWorksheet") . "/_rels/" . basename($fileWorksheet) . ".rels")) {
                                        ...
                                        if ($vmlRelationship != '') {
                                            ...
                                            $relsVML = simplexml_load_string(
                                                    // ~ http://schemas.openxmlformats.org/package/2006/relationships"
                                                    $this->securityScan($this->getFromZipArchive($zip, dirname($vmlRelationship) . '/_rels/' . basename($vmlRelationship) . '.rels')), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions());
                                            $drawings = array();
                                ...
                                if ($xmlSheet->drawing && !$this->readDataOnly) {
                                    foreach ( $xmlSheet->drawing as $drawing ) {
                                        ...
                                        $relsDrawing = simplexml_load_string(
                                                // ~ http://schemas.openxmlformats.org/package/2006/relationships"
                                                $this->securityScan($this->getFromZipArchive($zip, dirname($fileDrawing) . "/_rels/" . basename($fileDrawing) . ".rels")), 'SimpleXMLElement', \PHPExcel\Settings::getLibXmlLoaderOptions());
                                        $images = array();
