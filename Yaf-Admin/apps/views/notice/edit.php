	...
	<form id="fromID">
		<table class="content" border="0" cellspacing="0" cellpadding="0">
			...
			<tr>
                ...
                <td><textarea name="body" id="body" class="textarea" rows="3" cols="50">{{$detail.summary}}</textarea></td>
            </tr>
            <tr>
                <th class="left-txt">所属终端：</th>
                <td>
                    {{foreach $terminal as $val => $name}}
                    <label style="margin-right:20px;"><input {{if ($detail.terminal & $val) == $val}}checked="checked"{{/if}} type="checkbox" name="terminal" class="terminal" value="{{$val}}" />{{$name}}</label>
                    {{/foreach}}
                </td>
            </tr>
			<tr>
