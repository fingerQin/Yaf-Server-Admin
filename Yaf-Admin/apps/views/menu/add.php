...
<div class="main">
	<form name="myform" id="fromID" action="{{'Menu/add'|url}}" method="post">
		<table class="content" border="0" cellspacing="0" cellpadding="0">
			<tbody>
					...
					<td>
						<select name="parentid" class="form-control">
							<option value="0">作为一级菜单</option>
								...
								<option
								{{if $menu.menuid == $parentid}}selected="selected"{{/if}} value="{{$menu.menuid}}">{{$menu.menu_name}}</option>
								{{foreach $menu.sub as $sub_m}}
								<option {{if $sub_m.menuid == $parentid}}selected="selected"{{/if}} value="{{$sub_m.menuid}}">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;├─{{$sub_m.menu_name}}</option>
								{{foreach $sub_m.sub as $ss_m}}
								<option {{if $ss_m.menuid == $parentid}}selected="selected"{{/if}} value="{{$ss_m.menuid}}">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;│&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;├─{{$ss_m.menu_name}}</option>
								{{/foreach}}
							{{/foreach}}
							{{/foreach}}
						</select>
				...
				<tr>
					...
					<td>
						<input type="text" name="a" id="a" class="input-text" /> <span id="a_tip"></span></td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td><input type="radio" name="display" value="1" checked> 是 <input type="radio" name="display" value="0"> 否</td>
				</tr>
				<tr>
					<td></td>
					<td>
