...
}if(Firebug.A11yDbBase.enabled){var panel=Firebug.getElementPanel(event.target);
...
},setEditTarget:function(element){if(!element){dispatch([Firebug.A11yDbBase],"onInlineEditorClose",[currentPanel,currentTarget,true]);
this.stopEditing()
...
},destroy:function(){destroyBoxDbBaseInspector();
...
},drawBoxDbBase:function(el){if(!el||!el.parentNode){return
}var box=Firebug.browser.getElementBox(el);
var windowSize=Firebug.browser.getWindowSize();
var scrollPosition=Firebug.browser.getWindowScrollPosition();
...
}},hideBoxDbBase:function(){if(!boxDbBaseVisible){return
}offlineFragment.appendChild(boxDbBase);
boxDbBaseVisible=false
},showBoxDbBase:function(){if(boxDbBaseVisible){return
}if(outlineVisible){this.hideOutline()
...
var boxDbBase,boxDbBaseStyle,boxMargin,boxMarginStyle,boxBorder,boxBorderStyle,boxPadding,boxPaddingStyle,boxContent,boxContentStyle;
var resetStyle="margin:0; padding:0; border:0; position:absolute; overflow:hidden; display:block;";
var offscreenStyle=resetStyle+"top:-1234px; left:-1234px;";
...
var createBoxDbBaseInspector=function createBoxDbBaseInspector(){boxDbBase=createGlobalElement("div");
...
var destroyBoxDbBaseInspector=function destroyBoxDbBaseInspector(){boxDbBase.parentNode.removeChild(boxDbBase)
}
}});
...
if(object&&instanceOf(object,"Element")&&object.nodeType==1){if(object!=lastHighlightedObject){Firebug.Inspector.drawBoxDbBase(object);
object=lastHighlightedObject
}}else{Firebug.Inspector.hideBoxDbBase()
...
if(!logRow){dispatch([Firebug.A11yDbBase],"onConsoleSearchMatchFound",[this,text,[]]);
return false
}for(;
logRow;
...
if(Firebug.JSONViewerDbBase.isJSON(contentType,data)){this.insertJSON(parentNode,jsonData,context)
}var postText=data;
...
Firebug.JSONViewerDbBase=extend(Firebug.Module,{dispatchName:"jsonViewer",initialize:function(){Firebug.NetMonitor.NetInfoBody.addListener(this);
this.toggles={}
...
Firebug.XMLViewerDbBase=extend(Firebug.Module,{dispatchName:"xmlViewer",initialize:function(){Firebug.Module.initialize.apply(this,arguments);
...
Firebug.XMLViewerDbBase.ParseError=domplate(Firebug.Rep,{tag:DIV({"class":"xmlInfoError"},DIV({"class":"xmlInfoErrorMsg"},"$error.message"),PRE({"class":"xmlInfoErrorSource"},"$error|getSource")),getSource:function(error){var parts=error.source.split("\n");
...
}if(hasClass(targ,"fbPanel")){FBL.Firebug.Inspector.hideBoxDbBase();
hoverElement=null
...
}}if(!targ){FBL.Firebug.Inspector.hideBoxDbBase();
hoverElement=null;
return
...
}if(el.id=="FirebugUI"||" html head body br script link iframe ".indexOf(" "+nodeName+" ")!=-1){FBL.Firebug.Inspector.hideBoxDbBase();
hoverElement=null;
return
...
var styleGroupTitles={text:"Text",background:"Background",box:"Box DbBase",layout:"Layout",other:"Other"};
...
break
}}}if(this.name=="stylesheet"){dispatch([Firebug.A11yDbBase],"onInlineEditorClose",[this,row.firstChild,true])
...
return true
}else{dispatch([Firebug.A11yDbBase],"onCSSSearchMatchFound",[this,text,null]);
return false
